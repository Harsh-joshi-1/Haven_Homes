...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [token, setToken] = useState<string | null>(localStorage.getItem('Haven Homes_token'));
  const [isLoading, setIsLoading] = useState(true);

  // On mount, check if token exists and is valid
  useEffect(() => {
    const storedToken = localStorage.getItem('Haven Homes_token');
    const storedUser = localStorage.getItem('Haven Homes_user');
    if (storedToken && storedUser) {
      try {
        ...
        localStorage.removeItem('Haven Homes_user');
      }
    }
    setIsLoading(false);
  }, []);
  ...
  const login = useCallback(async (email: string, password: string, rememberMe: boolean = false) => {
    ...
    if (data.success && data.token) {
      localStorage.setItem('Haven Homes_token', data.token);
      localStorage.setItem('Haven Homes_user', JSON.stringify(data.user));
      setToken(data.token);
      setUser(data.user);
    } else {
