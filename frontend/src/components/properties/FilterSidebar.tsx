...
const FilterSidebar: React.FC<FilterSidebarProps> = ({ onFilterChange, isOpen = true }) => {
  ...
  const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const next = { ...activeFilters, [e.target.name]: e.target.value };
    setActiveFilters(next);
    onFilterChange(next);
  };
    ...
    return (
      <div className="border-b border-[#E6E0DA] py-6">
        ...
        >
          ...
          <div className="text-[#5A5856] group-hover:text-[#D4755B] transition-colors">
  ...
  return (
    <div className={`lg:w-80 h-full lg:sticky lg:top-24 bg-transparent lg:pr-8 pb-20 ${isOpen ? 'block' : 'hidden lg:block'}`}>
      ...
      <div className="flex items-center justify-between mb-8">
         ...
         <button 
           onClick={() => {
              const reset = { propertyType: [], facing: '', minPrice: '', maxPrice: '', bedrooms: '', bathrooms: '', amenities: [] };
              setActiveFilters(reset);
              onFilterChange(reset);
           }}
           className="font-red-hat text-[10px] font-bold text-[#D4755B] uppercase tracking-widest hover:underline"
         >
            Clear All
         </button>
      </div>
        ...
        <FilterSection title="Price Range">
           <div className="grid grid-cols-2 gap-3">
              <input 
                ...
                className="w-full bg-[#F8F6F6] border border-[#E6E0DA] rounded-xl px-4 py-3 font-red-hat text-sm focus:outline-none focus:ring-1 focus:ring-[#D4755B]"
              />
              <input 
                type="number" 
                name="maxPrice" 
                ...
                <button
                  ...
                  className={`flex items-center justify-center h-10 rounded-lg border transition-all text-xs font-red-hat font-bold ${
                    activeFilters.facing === f
                      ...
                      : 'bg-white border-[#E6E0DA] text-[#5A5856] hover:border-[#D4755B]'
                  }`}
                >
                  {f}
                </button>
           ...
           <div className="flex gap-2">
              {[2,3,4,5].map(b => (
                <button
                  key={b}
                  onClick={() => {
                    const next = { ...activeFilters, bedrooms: activeFilters.bedrooms === b.toString() ? '' : b.toString() };
                    setActiveFilters(next);
                    onFilterChange(next);
                  }}
                  className={`flex-1 h-12 rounded-xl border transition-all text-sm font-red-hat font-bold ${
                    activeFilters.bedrooms === b.toString()
                      ? 'bg-[#1C1B1A] border-[#1C1B1A] text-white'
                      : 'bg-white border-[#E6E0DA] text-[#5A5856] hover:border-[#1C1B1A]'
                  }`}
                >
